...
(function () {
  ...
  if (localStorage.getItem('minorkey-sound') === 'off') { PREFS.sound = false; localStorage.removeItem('minorkey-sound'); }
...
var CURATED = {};
  ...
  // adaptive-state helper (unused in this fork)
  ...
  function decorateStartHere(id) {
    ...
    picks.forEach(function (p, k) {
      ...
      var col = typeof gsc === 'function' ? gsc(p.source) : '#8A9BC4';
  ...
  var PAGE_MOTIFS = {
    ...
    'page-meditations': 'crescent', 'page-ledger': 'chart',
    'page-refrains': 'wave', 'page-practice': 'target'
  };
  function pageEnsoHTML(pageId) {
    return '<div class="page-enso">' +
      motifSVG(PAGE_MOTIFS[pageId], pageId, { w: 8, color: '#8A9BC4', style: 'width:100%;height:100%' }) +
      '</div>';
  }
  function addPageEnsos() {
    ...
    if (orn) {
      orn.innerHTML = '<span class="line"></span>' +
        motifSVG('torii', 'ornament', { color: 'rgba(138,155,196,0.6)', style: 'width:26px;height:26px' }) +
  ...
  function buildOverlay() {
    ...
    ov.innerHTML =
      '<div class="zen-bg"><div class="zen-blob b1"></div><div class="zen-blob b2"></div><div class="zen-blob b3"></div>' +
        '<div class="zen-enso">' + drawEnsoSVG('overlay-enso', { w: 2.4, gap: 0.9, color: 'var(--zen-c1,#8A9BC4)', style: 'width:86vmin;height:86vmin' }) + '</div>' +
  ...
  function openTap(idx) {
    ...
    function showIntro() {
      ...
      Z.zPhrase.innerHTML = motifSVG(motifFor(seq.icon), 'seq-' + seq.title, { color: 'var(--zen-c1,#8A9BC4)', style: 'width:.85em;height:.85em;vertical-align:-.08em' }) + '&nbsp; ' + seq.title;
  ...
  function buildReader() {
    ...
    reader.innerHTML =
      ...
      '<div class="page-enso reader-enso">' + ensoSVG('reader', { w: 8, color: '#8A9BC4', style: 'width:100%;height:100%' }) + '</div>';
  ...
  function openReader(kind, i) {
    ...
    if (kind === 'med') {
      ...
      h += '<h1 class="reader-title">' + motifSVG(motifFor(m.icon), 'med-' + m.day, { color: '#8A9BC4', style: 'width:26px;height:26px;margin-right:8px;vertical-align:-3px' }) + m.title + '</h1>';
  ...
  var GROUPS = [
    ...
    { id: 'concepts', label: 'Concepts', motif: null, enso: { w: 5.5, gap: 1.9, rot: 2.6 }, pages: ['concepts'] },
    { id: 'meds', label: 'Meditate', motif: 'crescent', pages: ['meditations'] },
    { id: 'refrains', label: 'Refrains', motif: 'wave', pages: ['refrains'] },
    { id: 'bill', label: 'Practice', motif: 'target', pages: ['practice'] }
  ];
  var PAGE_LABELS = { today: 'Today', concepts: 'Concepts', meditations: 'Meditations', refrains: 'Refrains', practice: 'Practice' };
  ...
  function activate(page) {
    ...
    if (page === 'refrains' && typeof renderRefrains === 'function') renderRefrains();
    if (page === 'practice' && typeof renderPractice === 'function') renderPractice();
  ...
  function createPages() {
    ...
    ['today', 'refrains', 'practice'].forEach(function (id) {
      var d = document.createElement('div');
  ...
  function renderToday() {
    ...
    var greet = slot === 'morning' ? 'Good morning, Bill.' : slot === 'day' ? 'Good afternoon, Bill.' : 'Good evening, Bill.';
    var mi = todayMedIndex(), m = MEDS[mi];
    var h = pageEnsoHTML('page-today') +
    ...
    h += '<div class="today-sec">' + (slot === 'evening' ? 'Tonight’s Contemplation' : 'Today’s Meditation') + '</div>' +
      '<div class="today-card glass" id="tdMed">' +
        ...
        '<div class="today-actions"><button class="tbtn" id="tdMedRead">Read</button></div>' +
      '</div>';

    if (typeof practiceCardsForToday === 'function') h += practiceCardsForToday(slot, now);
    ...
    if (typeof bindBillToday === 'function') bindBillToday(page);
  ...
  function renderLedger() {
    ...
    var h = pageEnsoHTML('page-ledger') +
      '<div style="margin-bottom:20px"><h2 style="font-family:\'Cormorant Garamond\',serif;font-size:24px;font-weight:400;color:#DCE2EF;margin-bottom:6px">The Ledger</h2>' +
  ...
  function openSheet() {
    ...
    data.innerHTML = '<div class="zset-label">Your data</div><div class="zset-row"><button id="zExport">Export backup</button><button id="zImport">Import</button></div><div class="zset-hint">A private JSON backup of your lines, notes and settings — kept only on this device unless you export it.</div>';
  ...
  function remapColors(val, mode) {
    ...
    val = val.replace(/#DCE2EF/gi, 'rgb(220,226,239)').replace(/#E2E7F2/gi, 'rgb(237,228,211)').replace(/#CBD6F0/gi, 'rgb(242,226,172)')
             .replace(/#090A0F/gi, 'rgb(9,10,15)').replace(/#07080C/gi, 'rgb(7,8,12)');
    if (mode === 'text') val = val.replace(/#8A9BC4/gi, 'rgb(138,155,196)').replace(/#7C90C0/gi, 'rgb(124,144,192)');
  ...
  function applyTheme() {
    ...
    if (meta) meta.setAttribute('content', light ? '#EEF1F7' : '#090A0F');
  ...
  var REMINDER_MSGS = {
    morning: { title: 'The refrain is waiting', body: 'One line before the scroll.' },
    evening: { title: 'The minor key', body: 'What was beautiful today because it ends?' }
  };
  ...
  function maybeInAppNudge() {
    ...
    var msg = hr < 12 ? 'Good morning — the refrain first?' : hr < 18 ? 'A pause, in the minor key?' : 'What was beautiful today because it ends?';
  ...
  function hookMeds() {
    ...
    document.querySelectorAll('.med-card').forEach(function (card, i) {
      card.onclick = function () { openReader('med', i); };
    });
  }
  ...
  function addFab() {
    ...
    fab.innerHTML = motifSVG('sun', 'fab', { color: 'rgba(16,20,32,0.78)', style: 'width:28px;height:28px' });
  ...
  var WK = 'minorkey-wave';
  var WAVE_STATES = [
    { v: 1, label: 'In the trough', short: 'Trough', tint: '#5A7B8C' },
    { v: 2, label: 'Low, still moving', short: 'Low', tint: '#6E7FA3' },
    { v: 3, label: 'Level', short: 'Level', tint: '#8A9BC4' },
    { v: 4, label: 'Rising', short: 'Rising', tint: '#7C90C0' },
  ...
  function waveSparkline() {
    ...
    var dots = pts.map(function (p) { return '<circle cx="' + p[0].toFixed(1) + '" cy="' + p[1].toFixed(1) + '" r="2.2" fill="var(--wave-tint,#8A9BC4)"/>'; }).join('');
    return '<svg class="wave-spark" viewBox="0 0 ' + W + ' ' + H + '">' +
      (pts.length > 1 ? '<polyline points="' + poly + '" fill="none" stroke="var(--wave-tint,#8A9BC4)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" opacity="0.7"/>' : '') +
      dots + '</svg>';
  }
  function applyWaveTint() {
    var t = waveToday();
    document.documentElement.style.setProperty('--wave-tint', t ? waveState(t.v).tint : '#8A9BC4');
  ...
  var TINT_SEL = '.map-hz, .map-q, .map-thread, .proto-block, .entry-week, .map-cadence-row, #tdEntry, .forge-card, .masc-card, .orisha-card, .orisha-teaching, .mirror-quote, .crisis-card, .excerpt-block, .eft-seq-card, .forge-protocol, .dtree-card, .drama-card, .ess-card, .mar-week';
  ...
  /* ================================================================
     ...
     ================================================================ */
  var KWK = 'minorkey-keptword', OKK = 'minorkey-oriki', AFK = 'minorkey-waters';
  ...
  function renderEssence() {
    ...
    var h = pageEnsoHTML('page-essence') +
      '<div style="margin-bottom:20px"><h2 style="font-family:\'Cormorant Garamond\',serif;font-size:24px;font-weight:400;color:#DCE2EF;margin-bottom:6px">' + ESSENCE.title + '</h2>' +
        ...
        '<div class="ess-body" id="essb-' + i + '">' +
          ...
          (s.deep ? '<button class="eft-read-link" data-essdeep="' + i + '">the deeper water ▾</button>' +
            '<div class="ess-deep" id="essd-' + i + '" style="display:none">' +
              ...
              (s.forYou ? '<div class="ess-foryou"><div class="ess-kicker" style="color:var(--wave-tint,#8A9BC4)">Held against your life</div>' + s.forYou + '</div>' : '') +
            '</div>' : '') +
  ...
  function renderWaters() {
    ...
    var h = pageEnsoHTML('page-waters') +
      '<div style="margin-bottom:18px"><h2 style="font-family:\'Cormorant Garamond\',serif;font-size:24px;font-weight:400;color:#DCE2EF;margin-bottom:6px">' + WATERS.title + '</h2>' +
      '<p style="font-size:12.5px;line-height:1.7;color:rgba(255,255,255,0.42)">' + WATERS.sub + '</p></div>';
    h += '<div class="wprin glass"><div class="ess-kicker" style="color:#8A9BC4">Principles before procedure</div>' +
  ...
  var TW_STEPS = [
    ...
    { t: 'Libation — optional', phrase: '“To those who came before — water on the ground.”', sub: 'A few drops of fresh water — never the salt — to earth or a living plant. Ground reaches ground.', btn: 'Continue', skip: true, color: WATER_BLUE },
  ...
  var MARIE_AS_COLOR = { human: '#8A9BC4', woman: '#E88BA0', lover: '#B85C38' };
  ...
  function renderUnusedWeekly() {
    var page = document.getElementById('page-marie');
    if (!page) return;
    var wi = marieWeekIdx(), w = MARIE.weeks[wi];
    var h = pageEnsoHTML('page-marie') +
      '<div style="margin-bottom:18px"><h2 style="font-family:\'Cormorant Garamond\',serif;font-size:24px;font-weight:400;color:#DCE2EF;margin-bottom:6px"></h2>' +
  ...
  /* bridge for bill.js (separate script, needs these helpers) */
  window.esc = esc; window.motifSVG = motifSVG; window.motifFor = motifFor;
  window.pageEnsoHTML = pageEnsoHTML; window.glassify = glassify; window.tintCards = tintCards;
